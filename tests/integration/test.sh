...
echo "=== NeoCoin Integration Tests ==="
...
echo "$METRICS" | grep -q "neocoin_chain_height" && echo "PASS: Metrics" || echo "FAIL: Metrics"
...
curl -sf http://127.0.0.1:8080/metrics | grep -E "^neocoin_" | head -3 > /dev/null && echo "PASS: Prometheus format" || echo "FAIL: Prometheus format"
