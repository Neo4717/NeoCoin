...
build:
	go build ${GOFLAGS} -o neocoin ./cmd/node
...
docker-build:
	docker build -t neocoin/blockchain:${VERSION} -t neocoin/blockchain:latest -f docker/Dockerfile .

# Docker build (reproducible)
docker-build-reproducible:
	docker build --build-arg VERSION=${VERSION} --build-arg BUILD_TIME=${BUILD_TIME} \
		-t neocoin/blockchain:${VERSION} -t neocoin/blockchain:latest -f docker/Dockerfile.reproducible .
